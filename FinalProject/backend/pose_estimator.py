...
# At the end of pose_estimator.py or any other test script

if __name__ == "__main__":
    import cv2 as cv
    # Ensure you have a valid image path or adjust the path accordingly
    test_image = cv.imread("path/to/your/test/image.jpg")
    if test_image is not None:
        pose_estimator = PoseEstimator()
        output_image = pose_estimator.get_pose_key_angles(test_image)
        cv.imshow("Test Pose Estimation", output_image)
        cv.waitKey(0)
